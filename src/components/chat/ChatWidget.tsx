...
export default function ChatWidget() {
  ...
  const [selectedFile, setSelectedFile] = useState<AttachmentType | null>(null);
  ...
  const syncToSupabase = async (msg: MessageData) => {
    if (!sessionId) return;
  ...
  const fetchAIResponse = async (
    userMsg: string,
    history: MessageData[],
    currentAttachment: AttachmentType | null = null,
  ) => {
    setIsTyping(true);
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        setSelectedFile({
          name: file.name,
          type: file.type,
          data: event.target.result as string, // Data URL (Base64)
        });
      }
    };
  ...
  const handleTalkToHuman = async () => {
    ...
    const connectingMsg: MessageData = {
      id: connectingMsgId,
  ...
  const initialOptions: string[] = t.raw("initialOptions") || [];
