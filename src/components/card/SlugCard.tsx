...
export default function SlugCard({ slug }: SlugCardProps) {
  const t = useTranslations("blog");
  const tCommon = useTranslations("common");
  const format = useFormatter();
  
  const [slugData, setSlugData] = useState<Article | null>(null);
  ...
  const authorImage = slugData?.author?.image
    ? urlForImage(slugData.author.image).width(200).url()
          ...
          <header className="mb-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="text-neutral-400 text-sm flex items-center gap-2">
                ...
                {loading ? "..." : formatDate(slugData?.publishedAt)}
              </div>
            </div>
